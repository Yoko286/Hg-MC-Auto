# 这个脚本是一个通用的地球化学仪器数据导出自动化工具，支持多语言界面、智能坐标捕获、文件搜索和命名策略配置等功能，旨在帮助用户快速创建适用于不同软件的数据导出自动化流程。
# 主要功能包括：
# 1. 多语言支持：提供中文和英文界面，用户可以根据需要；
# 2. 智能坐标捕获：用户可以通过移动鼠标并确认的方式捕获坐标，支持重试和验证，确保坐标准确；
# 3. 文件搜索器：根据用户指定的文件夹、扩展名和是否搜索子文件夹来查找数据文件，并提供文件列表供用户选择；
# 4. 文件名生成器：根据用户选择的命名策略（保持原名、添加时间戳、添加序号或自定义模板）来生成输出文件名，确保导出文件有清晰的命名规则；
# 5. 配置管理器：用户可以创建、保存、加载和删除软件配置档案，方便管理不同软件的导出流程；
# 6. 导出历史记录：自动记录每次批量导出的详细信息，包括处理的文件、成功和失败的统计数据，用户可以随时查看历史记录；
# 7. 错误处理和用户提示：在整个流程中提供清晰的提示信息和错误处理机制，帮助用户顺利完成配置和导出任务。
# 通过这个工具，用户可以轻松地为各种地球化学仪器软件创建定制化的数据导出自动化流程，大大提高工作效率和数据处理的规范性。
import os
import time
import json
import pyautogui
...
class LanguageManager:
    ...
    def _english_translations(self):
        return {
            ...
            "naming_options": "Keep original name [original.dat → original.csv],Add timestamp [original.dat → original_20231215_143022.csv],Add sequence number [original.dat → original_0001.csv],Custom template",
...
# ==================== 配置管理器 ====================
class ConfigManager:
    def __init__(self, config_dir="software_profiles"):
...
class FileSearcher:
    @staticmethod
    def find_files(folder_path: str, extensions: List[str], 
                   search_subfolders: bool = True) -> List[str]:
...
class FilenameGenerator:
    ...
    def generate_output_filename(self, source_filepath: str, index: int = None, 
                                total: int = None) -> str:
...
class AutomationExecutor:
    ...
    def execute_batch(self, batch_job: BatchJob) -> Dict[str, Any]:
        ...
        """
        results = {
            "total": len(batch_job.files_to_process),
            "success": 0,
            "failed": 0,
            "failed_files": [],
            "output_files": []
        ...
        for idx, source_file in enumerate(batch_job.files_to_process, 1):
            ...
            try:
                # 生成输出文件名
                output_filename = name_generator.generate_output_filename(
                    source_file, index=idx, total=len(batch_job.files_to_process)
                )
                
                # 确保文件名唯一
                if output_folder:
                    output_filename = FilenameGenerator.get_unique_filename(
    ...
    def _execute_single_export(self, input_file: str, output_file: str, 
...
class MultilingualExportAssistant:
    ...
    def batch_export(self):
        ...
        # 创建批量作业并执行
        batch_job = BatchJob(
            profile_name=profile.software_name,
            ...
            try:
                ...
                if format_choice == "0":
                    ...
                    config.source_extensions = [
                        ...
                        for ext in custom_formats.split(',')
                    ]
                    break
                else:
                    selected_indices = [int(idx.strip()) for idx in format_choice.split(',')]
                    selected_formats = [common_formats[idx-1] for idx in selected_indices 
                                      if 1 <= idx <= len(common_formats)]
                    if selected_formats:
    ...
    def _search_and_select_files(self, folder: str, config: FileProcessingConfig):
        ...
        print(self.lang.t('search_subfolders_status', 
                         status=self.lang.t('yes') if config.search_subfolders else self.lang.t('no')))
    ...
    def _preview_and_confirm(self, profile, source_folder, files, file_config):
        ...
        print(self.lang.t('search_subfolders_status', 
                         status=self.lang.t('yes') if file_config.search_subfolders else self.lang.t('no')))
        ...
        if files:
            ...
            for i in range(min(3, len(files))):
                output_name = name_gen.generate_output_filename(
                    files[i], index=i+1, total=len(files)
                )
    ...
    def _save_export_report(self, job: BatchJob, results: Dict):
        ...
        report_file = os.path.join(report_dir, 
                                  f"export_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
